...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-indigo-50 to-purple-50 dark:from-slate-900 dark:via-purple-900 dark:to-slate-900" itemScope itemType="https://schema.org/SoftwareApplication">
      {/* Animated Background */}
      <div className="absolute inset-0">
        {/* Gradient Overlays */}
        <div className="absolute inset-0 bg-gradient-to-r from-indigo-500/20 to-purple-600/20 dark:from-indigo-500/30 dark:to-purple-600/30"></div>
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-100/30 to-slate-200/60 dark:via-slate-900/50 dark:to-slate-900/80"></div>
        ...
        <div className="absolute inset-0 overflow-hidden">
          {images.map((image, index) => (
            <div
              key={index}
              className={`absolute opacity-20 hover:opacity-35 dark:opacity-25 dark:hover:opacity-40 transition-all duration-1000 ${image.className} ${isLoaded ? 'animate-fade-in' : 'opacity-0'}`}
              style={{
                left: `${10 + (index * 20)}%`,
                top: `${15 + (index * 15)}%`,
                transform: isLoaded 
            ...
            >
              <Image
                ...
                width={256}
                height={256}
                className="w-32 h-32 md:w-48 md:h-48 lg:w-64 lg:h-64 object-cover rounded-2xl shadow-2xl shadow-gray-500/20 dark:shadow-black/40 blur-sm hover:blur-none transition-all duration-500"
        ...
        <div className="absolute inset-0 pointer-events-none">
          {useMemo(() => 
            [...Array(15)].map((_, i) => (
              <div
                key={i}
                className="absolute w-1 h-1 md:w-2 md:h-2 bg-indigo-400/30 dark:bg-white/20 rounded-full animate-ping"
                style={{
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-center min-h-screen py-16 sm:py-24 lg:py-32 text-center">
          ...
          <div className="max-w-4xl mx-auto">
            {/* Badge */}
            <div className="mb-8 inline-flex items-center rounded-full bg-white/80 dark:bg-white/10 backdrop-blur-sm px-6 py-2 text-sm font-medium text-gray-800 dark:text-white ring-1 ring-inset ring-gray-300/50 dark:ring-white/20 animate-fade-in-up">
              <span className="mr-2">🚀</span>
              Revolutionizing HR Management
            </div>

            {/* Main Heading */}
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-gray-900 dark:text-white mb-8 animate-fade-in-up animation-delay-200" itemProp="name">
            ...
            {/* Description */}
            <p className="mt-6 text-xl md:text-2xl leading-relaxed text-gray-700 dark:text-gray-300 max-w-3xl mx-auto animate-fade-in-up animation-delay-400" itemProp="description">
            ...
            {/* Trust Indicators */}
            <div className="mt-12 flex flex-wrap justify-center items-center gap-8 text-sm text-gray-600 dark:text-gray-400 animate-fade-in-up animation-delay-600">
              <div className="flex items-center bg-white/60 dark:bg-white/5 backdrop-blur-sm rounded-full px-4 py-2 border border-gray-200/50 dark:border-white/10">
            ...
            <div className="mt-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in-up animation-delay-1000">
              {[
                ...
                <div
                  key={index}
                  className="group p-6 bg-white/60 dark:bg-white/5 backdrop-blur-sm rounded-2xl border border-gray-200/50 dark:border-white/10 hover:bg-white/80 dark:hover:bg-white/10 hover:border-gray-300/60 dark:hover:border-white/20 transition-all duration-300 hover:transform hover:scale-105"
                >
